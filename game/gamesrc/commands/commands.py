...
class CmdChapter(MuxCommand):
    """
    This command handles chapters.

    Usage:
        +chapter <chapter>
        +chapter/title <chapter> = <new title>
        +chapter/add <chapter> = <text to add>
        +chapter/del <chapter>

    This command handles creation and editing of chapters, which
    are units of text designed for pieces of literature.
    ...
    def func(self):
      ...
      # Assign the current caller
      caller = self.caller

      # Command expects an argument. If none, tip usage."
      if not self.args:
        caller.msg("Usage: +chapter <argument>")

      chapter = caller.search(self.ch_name)

      if not chapter:
        return

      if not chapter.access(caller, "edit"):
